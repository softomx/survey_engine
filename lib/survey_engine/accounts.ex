defmodule SurveyEngine.Accounts do
  @moduledoc """
  ...
  alias SurveyEngine.Accounts.{User, UserToken, UserNotifier}
  ...
  def get_user_by_company(company_id) do
  ...
  def register_user_with_company(attrs) do
    %User{}
    |> User.registration_changeset_with_company(attrs)
    |> Repo.insert()
  end
  ...
  def change_user_registration_with_company(%User{} = user, attrs \\ %{}) do
    User.registration_changeset_with_company(user, attrs,
      hash_password: false,
  ...
  """
  def confirm_user(token, user_params) do
    with {:ok, query} <- UserToken.verify_email_token_query(token, "confirm"),
         %User{} = user <- Repo.one(query),
         {:ok, %{user: user}} <- Repo.transaction(confirm_user_multi(user, user_params)) do
      {:ok, user}
    else
      _ -> :error
    end
  ...
  defp confirm_user_multi(user, user_params) do
    Ecto.Multi.new()
    |> Ecto.Multi.update(:user, User.confirm_changeset(user, user_params))
  ...
  def update_role_with_actions(%Role{} = role, actions) do
    role
    |> Role.changeset_role_actions(actions)
    |> Repo.update()
  end
