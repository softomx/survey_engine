defmodule SurveyEngineWeb.AgencyModelLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all agency_models", %{conn: conn, agency_model: agency_model} do
      {:ok, _index_live, html} = live(conn, ~p"/admin/catalogs/agency_models")
    ...
    test "saves new agency_model", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/admin/catalogs/agency_models")

      assert index_live |> element("a", "New Agency model") |> render_click() =~
               "New Agency model"

      assert_patch(index_live, ~p"/admin/catalogs/agency_models/new")
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, ~p"/admin/catalogs/agency_models")
    ...
    test "updates agency_model in listing", %{conn: conn, agency_model: agency_model} do
      ...
      assert_patch(index_live, ~p"/admin/catalogs/agency_models/#{agency_model}/edit")
  ...
  describe "Show" do
    ...
    test "displays agency_model", %{conn: conn, agency_model: agency_model} do
      {:ok, _show_live, html} = live(conn, ~p"/admin/catalogs/agency_models/#{agency_model}")
    ...
    test "updates agency_model within modal", %{conn: conn, agency_model: agency_model} do
      {:ok, show_live, _html} = live(conn, ~p"/admin/catalogs/agency_models/#{agency_model}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Agency model"

      assert_patch(show_live, ~p"/admin/catalogs/agency_models/#{agency_model}/show/edit")
      ...
      {:ok, _, html} =
        ...
        |> form("#agency_model-form", agency_model: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, ~p"/admin/catalogs/agency_models/#{agency_model}")
